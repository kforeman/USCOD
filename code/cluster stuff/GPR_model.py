...
# mc_draws =  20000       # number of total draws to make
# mc_burn =   10000       # number of draws to burn
# mc_length = 200         # final desired chain length
# mc_thin =   (mc_draws - mc_burn) / mc_length
...
parser =    OptionParser()
parser.add_option('-a', '--age', action='store', type='string', dest='age')
parser.add_option('-s', '--sex', action='store', type='int', dest='sex')
parser.add_option('-n', '--name', action='store', type='string', dest='name')
parser.add_option('-d', '--draws', action='store', type='int', dest='draws')
parser.add_option('-b', '--burn', action='store', type='int', dest='burn')
parser.add_option('-l', '--length', action='store', type='int', dest='length')
(options, args) =   parser.parse_args()
sex =       options.sex
age =       options.age
mod_name =  options.name
mc_draws =  options.draws
mc_burn =   options.burn
mc_length = options.length
mc_thin =   (mc_draws - mc_burn) / mc_length


### setup the data
# load in the csv
data =      mp.mlab.csv2rec(work_dir + '/model_inputs/downsampled.csv')
print 'Data loaded'
...
data =         mp.mlab.rec_append_fields(
                    rec =   data, 
                    names = 'year0', 
...
state_centers =     mp.mlab.csv2rec(work_dir + '/model_inputs/state_centers.csv')
...
cause_distances =   mp.mlab.csv2rec(work_dir + '/model_inputs/distances_' + str(sex) + '_' + age + '.csv')
...
'''
a matern euclidean of order 3/2:
C(d) = sigma^2 * (1 + sqrt(3)*d/p) * exp(-sqrt(3)*d/p)
'''
# find euclidean distance 
cause_euc =     np.array([[np.sqrt(np.sum([(cause_coords[i][d]-cause_coords[j][d])**2. for d in range(3)])) for j in causes] for i in causes])
# find scaled euclidean distance
@mc.deterministic
def cause_euc_sc(scale=rho_CI):
    return (1. + (np.sqrt(3.) * cause_euc / scale)) * np.exp(-1. * np.sqrt(3.) * cause_euc / scale)
'''
hmmm, something doesn't add up:
gp.matern.euclidean(cause_coords, cause_coords, diff_degree=3./2., amp=1., scale=1., symm=True)
should equal
(1. + (np.sqrt(3.) * cause_euc / scale)) * np.exp(-1. * np.sqrt(3.) * cause_euc / scale)
if scale = 1., but it doesn't...
'''
...
def SIC(sigma_SI=sigma_SI, rho_SI=rho_SI):
    return gp.matern.geo_deg(long_lat, long_lat, diff_degree=2., amp=sigma_SI, scale=rho_SI, symm=True)

# cause intercept covariance
@mc.deterministic
def CIC(sigma_CI=sigma_CI, rho_CI=rho_CI):
    return gp.matern.euclidean(cause_coords, cause_coords, diff_degree=2., amp=sigma_CI, scale=rho_CI, symm=True)

# state-cause interaction covariance
@mc.deterministic
def XIC(sigma_XI=sigma_XI, rho_XI=rho_XI):
    return gp.matern.geo_deg(long_lat, long_lat, diff_degree=2., amp=sigma_XI, scale=rho_XI, symm=True)

# state drift covariance
@mc.deterministic
def SSC(sigma_SS=sigma_SS, rho_SS=rho_SS):
    return gp.matern.geo_deg(long_lat, long_lat, diff_degree=2., amp=sigma_SS, scale=rho_SS, symm=True)

# cause drift covariance
@mc.deterministic
def CSC(sigma_CS=sigma_CS, rho_CS=rho_CS):
    return gp.matern.euclidean(cause_coords, cause_coords, diff_degree=2., amp=sigma_CS, scale=rho_CS, symm=True)
...
model.sample(iter=mc_draws, burn=mc_burn, thin=mc_thin, verbose=True)
#model.sample(100)
...
output =            mp.mlab.rec_append_fields(  rec =   data, 
                        names = ['mean', 'lower', 'upper'], 
                        arrs =  [mean_estimate, lower_estimate, upper_estimate])
mp.mlab.rec2csv(output, work_dir + '/model_outputs/' + mod_name + '_' + str(sex) + '_' + age + '.csv')

# save draws
draws =     mp.mlab.rec_append_fields(
                    rec =   data,
                    names = ['draw_' + str(i+1) for i in range(mc_length)],
                    arrs =  [model.trace('estimate')[i] for i in range(mc_length)])
mp.mlab.rec2csv(draws, work_dir + '/model_outputs/' + mod_name + '_draws_' + str(sex) + '_' + age + '.csv')
