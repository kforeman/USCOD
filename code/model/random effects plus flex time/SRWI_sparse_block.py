...
Purpose:	fit smoothed RW model, adding on state/cause interactions
'''
...
def intercept_sc(B0_sc=B0_sc):
    return state_cause_indices_sp.dot(np.concatenate(B0_sc))
...
for c in causes:
    mc.MAP([B0_sc[c], data_likelihood]).fit(method='fmin_powell', verbose=1)
