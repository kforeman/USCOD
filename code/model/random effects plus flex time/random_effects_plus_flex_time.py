...
Purpose:	fit model with random intercepts and slopes by cause and state plus flexible time model
'''
...
### setup spline evaluation
# distance between knots for temporal B splines
knot_spacing =  5

# find years for both evaluation and sampling
years =         np.arange(np.min(data.year),    np.max(data.year)+1,            1)
year_list =     dict([(y, i) for i, y in enumerate(years)])
syears =        np.arange(np.min(data.year),    np.max(data.year)+knot_spacing, knot_spacing)
syear_list =    dict([(y, i) for i, y in enumerate(syears)])

# index years
year_indices =  np.array([data.year == y for y in year_list])

# index by state and year
state_year_list =       dict([(sy, i) for i, sy in enumerate([(s,y) for s in state_list for y in year_list])])
state_year_indices =    np.array([year_indices[year_list[sy[1]]] & state_indices[state_list[sy[0]]] for sy in state_year_list])

# find sample years by state
state_syear_list =      dict([(sy, i) for i, sy in enumerate([(s,y) for s in state_list for y in syear_list])])
state_syear_lookup =    

# index by cause and year
cause_year_list =       dict([(cy, i) for i, cy in enumerate([(c,y) for c in cause_list for y in year_list])])
cause_syear_list =      dict([(cy, i) for i, cy in enumerate([(c,y) for c in cause_list for y in syear_list])])
cause_year_indices =    np.array([year_indices[year_list[cy[1]]] & cause_indices[cause_list[cy[0]]] for cy in cause_year_list])

# function to interpolate over time
def spline_interpolation(values, x_out=years, x_in=syears):
    return interp.splev(x_out, interp.splrep(x_in, values))
