...
Purpose:	fit smoothed RW with interactions model

model upload params:
formula: 
    y_{s,c,t} ~ \\exp(\\alpha + \\gamma \\times t + exposure + \\beta_{s} + \\beta_{c} + \\hat \\beta_{s,c} + \\hat d_{s} \\times t + d_{c} \\times t + \\sum_{n=0}^t(u_{s,n}) + \\sum_{n=0}^t(u_{c,n}))
name:
    Flexible Time Model
comments:
    Random walks with drift on both cause and state. Offsets by state, cause, and state-cause interaction.
...
'''
    ...
    y[s,c,t=n]  ~ exp(alpha + gamma*n + exposure + B[s] + B[c] + B[s,c] + d[s]*n + d[c]*n + sum(u[s,t=0:n]) + sum(u[c,t=0:n]))
    ...
    hyperpriors
        ...
        B[s]    : random intercept by state
            sigma_b_s   ~ U(0, 1e2)
        
        B[c]    : random intercept by cause
            sigma_b_c   ~ U(0, 1e2)
        
        B[s,c]  : random intercept by state/cause interaction
            sigma_b_sc  ~ U(0, 1e2)
...
def intercept_s(B_s=B_s):
    return  state_indices_sp.dot(B_s)
